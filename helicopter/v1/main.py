...
def generate_frame(symbol: str) -> list: # Cоздаем кадр
    frame = []
...
def paint_land(frame: list, level: int) -> None: # Cоздаем линию земли
    frame[level - 1] = ''.join(['_' * LETTER_COUNT])


def prepare_helicopter(x: int, y: int) -> list: # Выравнинваем позицию вертолета (его вн вид)
...
def paint_helicopter(frame: list, x: int, y: int) -> None: # Рисуем вертолет
    copter = prepare_helicopter(x, y)
...
def get_copter_positions(track: list) -> list: # Получаем координаты вертолета
    points = []
...
def print_frames(track: list, land: int) -> None: # Рисуем передвиженеие
